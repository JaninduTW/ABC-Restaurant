  ...
  return (
    <div>
      ...
      <div className='text-center'>
        <p className='text-[#000000] font-bold py-3 text-3xl pb-2 pt-[5.5rem] bg-white'>Order Foods</p>
      </div>
          ...
          {filteredItems.length ? (
            filteredItems.map((item) => (
              <div key={item.id} className='w-full shadow-xl flex flex-col p-4 my-6 rounded-lg hover:scale-105 duration-300'>
                ...
                <button 
                  className='bg-[#000000] w-[200px] rounded-md font-medium my-6 px-6 mx-auto py-3 text-white'
                  disabled={item.availability === 'No'}
                  onClick={() => addToCart(item)}
                >
                  {item.availability === 'Yes' ? 'Add to cart' : 'Not Available'}
                </button>
              </div>
            ))
          ) : (
